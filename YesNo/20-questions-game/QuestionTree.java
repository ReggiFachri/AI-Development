...
public class QuestionTree {
	...
	private QuestionNode play(QuestionNode node) {
		...
		if (node.question.startsWith("A:")) {
			// Prints out the answer node data in a formatted manner to the desired
			// PrintStream.
			output.print("Would your object happen to be " + node.question.substring(2) + "? ");
			// If the guess was correct, prints out an appropriate message and increments
			// gamesWon.
			if (UserInterface.nextAnswer(input)) {
				output.println("I win!");
				gamesWon++;
			}
			// If the guess was incorrect, prompts the user for more information to alter
			// the tree.
			else {
				output.print("I lose. What is your object? ");
				String object = "A:" + input.nextLine();
				output.print(
						"Type a yes/no question to distinguish your item from " + node.question.substring(2) + ": ");
				String newQuestion = "Q:" + input.nextLine();
				output.print("And what is the answer for your object? ");
				// Appropriately adjusts the tree based on if the additional object should be
				// added to the left
		...
		// If the node is a question node, then prints out the question and recursively
		// calls
		// the method based on if the user enters yes (traverse down left tree) or no
		// (traverse
		// down right tree).
		else {
